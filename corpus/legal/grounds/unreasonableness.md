---
id: unreasonableness
name: Legal unreasonableness
plainName: The decision was one no reasonable decision-maker could make
oneLine: >-
  Even with power, a decision can be unlawful if it lacks any sensible
  justification.
whatItMeans: >-
  Even where a decision-maker has the power, the decision can be unlawful if it
  lacks an evident and intelligible justification on the material before them,
  or is so unreasonable that no reasonable decision-maker could have reached it.
  This is a high bar — it is not enough to disagree.
plainExample: >-
  A penalty far out of proportion to a tiny, first-time mistake, with no
  explanation.
whatRelates:
...
elements:
  - id: justification
    ...
    layPrompt: >-
      Does the outcome seem to make no sense on the facts the decision-maker
      had?
  - id: disproportionate
...
leadingCases:
  - name: Associated Provincial Picture Houses Ltd v Wednesbury Corporation
    ...
    explains: >-
      ...
      decision was so unreasonable that no reasonable authority could ever have
      come to it. Australian law is no longer limited to that question.
...
**Wednesbury marked as superseded, 2026-08-23, on the owner's instruction.** Wednesbury stays,
because it is where the ground began and readers meet the name everywhere. But it was published
here as though it still stated the test, and it does not. The owner's instruction: since Li, legal
unreasonableness in Australia is no longer limited to Wednesbury unreasonableness.

The word the owner first used was "overruled". The entry does not use it, because it would not be
right: Wednesbury unreasonableness is still one way a decision can be legally unreasonable — it is
no longer the boundary. That is the owner's own second formulation, and it is the one published.

The cases are now in sequence — Wednesbury, then Li, then SZVFW, then DUA16 — so the supersession
reads in order rather than needing to be assembled from a list. Li's note carries the ratio in
lay terms (a decision that lacks an evident and intelligible justification) and Gageler J's point
that the ground reaches how a decision is made, not only what was decided.

**No judge is named in Li's note**, for the reason recorded in subjective-jurisdictional-fact:
