...
- **OCR:** provider-pluggable, IN MEMORY, **default disabled** (`lib/intake/ocr.ts`). Paste-text always works; with no `OCR_PROVIDER`+`OCR_API_KEY` the upload path returns `ocr-unavailable` and asks the person to paste. GCV wired as the first provider.
...
- **Icons:** `public/icon-*.png` are generated placeholders (`scripts/make-icons.mjs`) — replace with a designed icon before launch.
