...
export function GroundExplainer({ ground: g, level = "h2" }: { ground: Ground; level?: "h1" | "h2" }) {
  ...
  return (
    <article className="space-y-5">
      <div>
        <p className="text-[12px] uppercase tracking-[0.2em] text-accent">Ground of review</p>
        <Heading className={`mt-2 font-display font-bold text-ink ${level === "h1" ? "text-[32px] sm:text-[42px] leading-[1.06]" : "text-[24px]"}`}>
          {g.plainName}
        </Heading>
        <p className="mt-1 text-[14px] uppercase tracking-[0.1em] text-ink-faint">{g.name}</p>
      ...
      <div className="rounded-card border border-line bg-sand-surface px-4 py-3">
        <h3 className="text-[12px] font-semibold uppercase tracking-[0.16em] text-accent">A plain example</h3>
        <p className="mt-1.5 text-[16px] leading-[1.6] text-ink-soft">{g.plainExample}</p>
      </div>

      <div>
        <h3 className="text-[12px] font-semibold uppercase tracking-[0.16em] text-accent">What might relate to this</h3>
      ...
      {g.whatItIsNot ? (
        <div className="rounded-card border-l-[3px] border-gold bg-gold-soft px-4 py-3">
          <h3 className="text-[12px] font-semibold uppercase tracking-[0.16em] text-gold">What this does not mean</h3>
          <p className="mt-1.5 text-[16px] leading-[1.6] text-ink">{g.whatItIsNot}</p>
        </div>
      ) : null}
