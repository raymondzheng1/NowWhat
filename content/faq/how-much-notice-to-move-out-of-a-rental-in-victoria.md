---
title: >-
  How do I check whether a notice to vacate is valid in Victoria?
description: >-
  Learn what makes a notice to vacate valid in Victoria, what to check on yours,
  and how to get free help if something looks wrong.
question: >-
  How do I check whether a notice to vacate is valid in Victoria?
answer: >-
  In Victoria, a notice to vacate must use the correct form, give a valid legal
  reason, and include the right notice period for that reason. If the notice
  period is too short for the reason given, the notice may not be valid.
entryId: vic-renting
category: Renting
sources:
  - Residential Tenancies Act 1997 (Vic) — legislation.vic.gov.au
  - >-
    Consumer Affairs Victoria — Challenging a notice to vacate; Challenging rent
    increases or high rent — consumer.vic.gov.au
  - VCAT — Residential tenancies — vcat.vic.gov.au
  - Rental Dispute Resolution Victoria — rdrv.vic.gov.au
  - Tenants Victoria — tenantsvic.org.au
related:
  - challenge-a-notice-to-vacate-in-victoria
  - what-happens-at-a-vcat-hearing
updated: '2026-06-30'
---

## What the law requires

A notice to vacate in Victoria must meet three things:

- It must be on the **correct, up-to-date form**.
- It must give a **valid legal reason** for asking you to leave.
- It must give a **notice period that matches the reason** — if the period is too short, the notice may not be valid.

If any of these things are wrong or missing, the notice may not hold up.

## Reasons a notice might not be valid

Your notice may have a problem if:

- It uses the wrong form or an out-of-date form.
- It is missing required information.
- The notice period is shorter than the law allows for that reason.
- The reason on the notice is not a lawful reason.
- The reason is not genuine — for example, it says "sale" but there is no real plan to sell.
- You were given the notice because you asked for repairs or stood up for your rights.

## What to check straight away

Gather these things and look them over carefully:

- The notice itself — check the date, the form used, and the reason given.
- Your rental agreement (lease) and any condition report.
- Your rent payment records.
- Any messages with your rental provider or agent, including when you got the notice.
- Photos of the condition of the place.
- The envelope or postmark showing when the notice arrived.

### If the time limit has already passed

Missing the date does not always end things. A tribunal can be asked to allow a late application. It does not have to say yes, but it can be asked, and it is worth raising as soon as you can. A human legal service can help you explain why you were late.

## How to get free help

There are time limits involved, so it helps to act early. You can [start by finding out about your options here](/start).

Free help is available from:

- **Tenants Victoria** — Rental Support Line **(03) 9416 2577**
- **Rental Dispute Resolution Victoria (RDRV)** — **(1300 017 378)**
- **Victoria Legal Aid** — Legal Help line **(1300 792 387)**
- **Consumer Affairs Victoria** — free renting information

You can also apply to **VCAT** to challenge a notice to vacate. If you miss the early deadline, you may still be able to argue the notice is not valid at a possession hearing, or ask for more time.

<!--
Retitled 2026-08-23 on the owner's QA decision. The page was called "How much notice does my rental
provider have to give me to move out in Victoria?" and never stated a single period for a single
reason — it says four times that the period must match the reason on the notice.

The owner's options were to publish sourced notice periods or retitle. The periods are not in the
app's sourced material, so the title now describes what the page actually does. The slug is
deliberately unchanged: it carries the search traffic, and breaking it would cost more than the
mismatch does.

If notice periods are sourced later, restore the original title and answer the question directly.
-->
